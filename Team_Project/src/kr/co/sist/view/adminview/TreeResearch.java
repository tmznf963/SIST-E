...
public class TreeResearch extends DefaultMutableTreeNode {
	private Vector<EmpVO> v;
	public TreeResearch(Vector<EmpVO> v) {
this.v=v;
		this.setUserObject("연구부");
		createTree();
	}
	private void createTree() {
		if(v!=null) {
		for(int i =0; i<v.size(); i++) {
			if(v.get(i).getDname().equalsIgnoreCase("연구부")) {
