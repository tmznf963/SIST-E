...
public class TreePublicRelations extends DefaultMutableTreeNode {
	private Vector<EmpVO> v;
	public TreePublicRelations(Vector<EmpVO> v) {
this.v=v;
		this.setUserObject("홍보부");
		createTree();
	}
	private void createTree() {
		if(v!=null) {
		for(int i =0; i<v.size(); i++) {
			if(v.get(i).getDname().equalsIgnoreCase("홍보부")) {
