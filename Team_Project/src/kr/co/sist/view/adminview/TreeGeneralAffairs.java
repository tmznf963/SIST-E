...
public class TreeGeneralAffairs extends DefaultMutableTreeNode {
	private Vector<EmpVO> v;
	public TreeGeneralAffairs(Vector<EmpVO> v) {
		this.v=v;
		this.setUserObject("총무부");
		createTree();
	}
	private void createTree() {
		if(v!=null) {
		for(int i =0; i<v.size(); i++) {
			if(v.get(i).getDname().equalsIgnoreCase("총무부")) {
